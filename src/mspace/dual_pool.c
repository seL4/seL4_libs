...
void mspace_dual_pool_create(mspace_dual_pool_t *dual_pool, struct mspace_fixed_pool_config config)
{
    *dual_pool = (mspace_dual_pool_t) {
        .fixed_pool_start = (uint32_t)config.pool,
        .fixed_pool_end = (uint32_t)config.pool + config.size,
        .have_virtual_pool = 0
    };
...
void *_mspace_dual_pool_alloc(allocman_t *alloc, void *_dual_pool, uint32_t bytes, int *error)
{
...
{
    mspace_dual_pool_t *dual_pool = (mspace_dual_pool_t*)_dual_pool;
    if ( (uint32_t)ptr >= dual_pool->fixed_pool_start && (uint32_t)ptr < dual_pool->fixed_pool_end) {
